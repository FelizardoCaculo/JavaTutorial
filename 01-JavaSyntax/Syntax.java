public class Syntax {
    /*
     * 1- Every line of code that runs in Java must be inside a 
     * class.
     * In our example, we named the class Syntax. A class should
     * always start with an uppercase first letter.
     * 2 - Note: Java is case-sensitive: "MyClass" and "myclass" has
     * different meaning.
     * 3 - The name of the java file must match the class name. 
     * When saving the file, save it using the class name and add ".
     * java" to the end of the filename.
     * 4 - The main() method is required and you will see it in 
     * very Java program.
     * 5 - Any code inside the main() method will be executed. Don't 
     * worry about the keywords before and after main. You will get 
     * to know them bit by bit while reading this tutorial.
     * 6 - For now, just remember that every Java program has a 
     * class name which must match the filename, and that every 
     * program must contain the main() method.
     */
    public static void main(String[] args){
        System.out.println("\nOlá mundo!\n");
        System.out.println("Seja muito bem vindo ao curso Java!\n");
    }
}